...
export function ItemRow(props: ItemRowProps) {
  ...
  return (
    <div className="group">
      <div className={`flex items-center gap-2 p-2 rounded border border-slate-800 ${item.checked ? 'opacity-60' : ''} ${isLocked ? 'bg-slate-900/40' : ''}`}>
        <input
          type="checkbox"
        ...
        />
        <button
          className={`flex-1 text-left ${item.checked ? 'line-through' : ''} ${isLocked ? 'cursor-not-allowed text-slate-500' : ''}`}
      ...
      {isLocked && item.lockReason && <p className="pl-2 mt-1 text-xs text-amber-300">{item.lockReason}</p>}
