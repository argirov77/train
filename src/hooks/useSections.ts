...
export function useSections() {
  ...
  const refetch = useCallback(async () => {
    ...
    if (fetchError) {
      ...
      if (!userId) {
        ...
        const itemIds = normalized.flatMap((section) => section.topics.flatMap((topic) => topic.items.map((item) => item.id)));
        if (itemIds.length === 0) {
          setSections(normalized);
        } else {
            ...
            normalized.map((section) => ({
              ...
              topics: section.topics.map((topic) => ({
                ...
                items: topic.items.map((item) => {
                  ...
                  return progress
                    ? { ...item, checked: progress.is_completed, checked_at: progress.completed_at }
                    : item;
                }),
              })),
            })),
  ...
  const getNextPosition = async (table: 'sections' | 'topics' | 'items', field?: { key: string; value: string }) => {
  ...
  const addSection = async (title: string, description?: string) => {
    ...
    const { error: insertError } = await supabase.from('sections').insert({ title, description, position });
  ...
  const addTopic = async (sectionId: string, title: string, description?: string) => {
    if (!supabase) return;
    const position = await getNextPosition('topics', { key: 'section_id', value: sectionId });
    const { error: insertError } = await supabase.from('topics').insert({ section_id: sectionId, title, description, position });
    if (insertError) throw insertError;
    await refetch();
  };
  ...
  return { sections, setSections, loading, error, refetch, addSection, addTopic, deleteSection, deleteTopic };
}
